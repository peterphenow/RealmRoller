...
button.on("click", function () {
  ...
  if (listOfListofNames.length === 0) {
    ...
    if (
      listOfListofNames.filter((list) => list.category === userInput.race)
        .length === 0
    ) {
      //create list if none exists
      getName(userInput.race, userInput.gender);
    } else {
      let index = listOfListofNames.findIndex(
        (listOfNames) => listOfNames.category === userInput.race
      );
      if (
        listOfListofNames[index].nextNameIndex ===
        listOfListofNames[index].list.length - 1
      ) {
      ...
      localStorage.setItem(
        "listOfListOfNames",
        JSON.stringify(listOfListofNames)
      );
    }
  }
});
...
function getName(race, gender) {
  ...
  let queryURL =
    "https://api.fungenerators.com/name/generate?api_key=" +
    api_key +
    "&variant=" +
    gender +
    "&limit=100&category=" +
  ...
  $.ajax({
    ...
    .fail(function (response) {
      console.log(
        "API call to name generator errored - error code " +
          response.responseJSON.error.code
      );
    })
    .done(function (response) {
      console.log("API call made");
      listOfListofNames.push({
        category: race,
        nextNameIndex: 0,
        list: response.contents.names,
      });
      let index = listOfListofNames.length - 1;
      createNPC(
        listOfListofNames[index].list[listOfListofNames[index].nextNameIndex]
      );
      listOfListofNames[index].nextNameIndex++;
      localStorage.setItem(
        "listOfListOfNames",
...
function getWealth() {
  ...
  } else if (rndWealth < 15) {
    //poor
    return wealthLevels[1];
  } else if (rndWealth < 65) {
    //modest
    return wealthLevels[2];
  } else if (rndWealth < 80) {
    //well-off
    return wealthLevels[3];
  } else if (rndWealth < 95) {
    //wealthy
